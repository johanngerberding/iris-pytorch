...
class VectorQuantization(torch.autograd.Function): 
    ...
    def forward(ctx, inputs: torch.Tensor, codebook: torch.Tensor) -> torch.Tensor:
        with torch.no_grad():
            ...
            distances = torch.addmm(codebook_sqr + inputs_sqr, inputs_flatten, codebook.t(), alpha=-2.0, beta=1.)
    ...
    def backward(ctx, grad_output):
        raise RuntimeError("No gradient for VectorQuantization")
    

class VectorQuantizationStraightThrough(torch.autograd.Function):
    @staticmethod
    def forward(ctx, inputs: torch.Tensor, codebook: torch.Tensor) -> tuple:
